...
import androidx.recyclerview.widget.RecyclerView;
 ...
 class HorizontalScrollListener extends RecyclerView.OnScrollListener implements RecyclerView.OnItemTouchListener {
    ...
    private VerticalScrollListener2 mVerticalScrollListener2;
    private boolean mIsHorizontallyMoving=false;

    public HorizontalScrollListener(SmartTableView2 smartTableView2) {
        ...
        mHeaderRv=mSmartTableView2.getHeaderRecyclerView();
         ...
         mVerticalScrollListener2=smartTableView2.getVerticalScrollListener2();
    }
...
//                        Log.v("TTT","horizontal  mContentRV.removeOnScrollListener");

//                        Log.v("TTT","  mHeaderRv.removeOnScrollListener");
                        mHeaderRv.stopScroll();
//                        mVerticalScrollListener2.removeScrollListener(mLastTouchedRv==mContentRV);
                    }
                }else {
                    int lastTouchedIndex=getLastTouchedIndex(rv);

                    if (mContentLayoutManager==null){
                        Log.v("TTT","  layoutManager==null");
                            return false;
                    }
...
//                        Log.v("TTT","  mContentLayoutManager.getChildAt(lastTouchedIndex)");
...
//                Log.v("TTT","horizontal MotionEvent.ACTION_UP rv.removeOnScrollListener");
...
//            Log.v("TTT","horizontal MotionEvent.ACTION_CANCEL rv.removeOnScrollListener");
...
//            Log.v("TTT","  recyclerView != mHeaderRv ");
    ...
    public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
        ...
        if (newState==RecyclerView.SCROLL_STATE_DRAGGING || newState==RecyclerView.SCROLL_STATE_SETTLING){
            mIsHorizontallyMoving=true;
        }else {
            mIsHorizontallyMoving=false;
        }
//        Log.v("TTT","onScrollStateChanged state is ="+recyclerView.getScrollState());
    }
    ...
    private void renewScrollPosition(@NonNull RecyclerView recyclerView) {
        ...
        if (isRTL) {
//            Log.v("TTT","getLayoutDirection is RTL");
            int sidebarEdgeLeft=0;
            if (mSmartTableView2.sideBarIsShowing()) {
                sidebarEdgeLeft = mSmartTableView2.getSidebarRecyclerView().getLeft();
            }else {
